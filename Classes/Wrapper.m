...
// to add a method of Wrapper class, we use category, which name is Private
...
- (id)init
{
    if(self = [super init])
    {
		//LN: in h file, we only have pointer for receivedData
	   ...
	   withParameters:(NSDictionary *)parameters
{
	...
	//LN it will alloc the string and give pointer to *contentType
    ...
    if (parameters != nil)
    {
        ...
        {
			//LN question: key is id data type, why can it use NSString method ?
			// we translate key to UTF8, and rename it to *encodeKey
            NSString *encodedKey = [key stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
			
			// we generate a String which is named value, we output key of parameters
            CFStringRef value = (CFStringRef)[[parameters objectForKey:key] copy];
			
            // Escape even the "reserved" characters for URLs 
            // as defined in http://www.ietf.org/rfc/rfc2396.txt
			//LN we get the encodedValue which mapping to key-value
            CFStringRef encodedValue = CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, 
                                                                               value,
                                                                               NULL, 
                                                                               (CFStringRef)@";/?:@&=+$,", 
                                                                               kCFStringEncodingUTF8);
			//LN params is combined by encodedkey and encodedValue
...
- (void)startConnection:(NSURLRequest *)request
{
    if (asynchronous)
    {
		...
		//LN we sill assign delegate to Wrapper object
